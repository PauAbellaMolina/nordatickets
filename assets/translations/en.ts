export const en: { [key: string]: string } = {
  days: 'days',
  hours: 'hours',
  minutes: 'minutes',
  seconds: 'seconds',
  event: 'Event',
  events: 'Events',
  addEventByQrExplanation: 'Add an event by scanning its QR code',
  stopFollowingEventQuestion: 'Stop following this event?',
  stopFollowingEventConfirmation: 'Yes, stop following',
  ticketsBoughtWillShowHereExplanation: 'Tickets bought will show here',
  noTicketsInWallet: 'No tickets in wallet',
  profile: 'Profile',
  infoAboutYou: 'Info about you',
  emailAndPaymentMethods: 'Email and payment method',
  cardWithExpiryDate: 'Card with expiry date',
  delete: 'Delete',
  purchaseReceipts: 'Purchase receipts',
  purchaseReceiptsDetails: 'Purchase receipt details',
  refundReceiptsDetails: 'Refund receipt details',
  helpAndFaqs: 'Help and FAQs',
  termsAndPrivacy: 'Terms and conditions',
  haveReadAndAcceptTermsAndPrivacy: 'I have read and accept the terms and conditions',
  getInContactExplanation: 'If you\'ve had any problems using Norda TIckets, you can contact us from the same email address with which you are registered.',
  sendAnEmailTo: 'Send an email to',
  changeLanguage: 'Change language',
  noAccountQuestion: 'Don\'t have an account?',
  alreadyAccountQuestion: 'Already have an account?',
  signUp: 'Sign up',
  signMeUp: 'Sign me up',
  accountCreation: 'Account creation',
  createAccount: 'Create account',
  tryAgain: 'Try again',
  logIn: 'Log in',
  logOut: 'Log out',
  email: 'Email',
  birthdate: 'Date of birth',
  oneTimeCode: 'One-time code',
  emailCodeExplanation: 'We will send a 6-digit code to your email',
  send: 'Send',
  enter: 'Enter',
  selling: 'Selling',
  notSelling: 'Not selling',
  ticketsAddedToWallet: 'Tickets added to wallet',
  ticketsSentToEmail: 'Tickets sent to email',
  cart: 'Cart',
  continueWithCart: 'Continue with the cart',
  serviceFee: 'Service fee',
  usingCreditCard: 'Using credit card',
  usingCardWithExpiryDate: 'Using card with expiry date',
  saveCardForFuturePurchases: 'Save card for future purchases',
  saveCardForFuturePurchasesSubtext: '(Except Apple Pay and Google Pay)',
  notAvailable: 'Not available',
  buy: 'Buy',
  total: 'Total',
  ticketReedemableFor: 'Ticket redeemable for',
  ticketRefundedFor: 'Refund of the ticket redeemable for',
  ofTheInvoice: 'of the invoice',
  paymentProcessing: 'Payment processing',
  paymentFailed: 'Payment failed',
  walletTicketAddonExplanation: 'Use it when you pick up a drink',
  activable: 'Activable',
  payment: 'Payment',
  activateTicket: 'Activate ticket',
  activateTicketRefundableAddonExplanation: 'You can return the cup before the event ends',
  activateTicketNonRefundableAddonExplanation: 'You won\'t be able to return the cup',
  eventTicketRefundableAddonExplanation: 'Returnable until the event ends',
  eventTicketNonRefundableAddonExplanation: 'Not returnable',
  loading: 'Loading',
  clickIfNoRedirectExplanation: 'If you are not redirected to the payment screen automatically, click the following button.',
  clickToGoToPaymentExplanation: 'Click the following button to go to the payment screen.',
  goToPaymentScreenBtn: 'Go to payment',
  simplifiedInvoice: 'Simplified invoice',
  refundInvoice: 'Refund invoice',
  issuer: 'Issuer',
  address: 'Address',
  cityAndCountry: 'City and country',
  contact: 'Contact',
  invoiceNumber: 'Invoice number',
  transactionDate: 'Transaction date',
  refundDate: 'Refund date',
  orderIdentifier: 'Order ID (TPV)',
  receiptOrderStatus: 'Order status (TPV)',
  description: 'Description',
  amount: 'Amount',
  quantity: 'Quantity',
  deactivateTicketConfirmationQuestion: 'Are you sure about deactivating this ticket?',
  deactivateTicket: 'Deactivate ticket',
  yesDeactivate: 'Yes, deactivate',
  ticketActive: 'Ticket active',
  deactivateTicketOnDrinkExplanation: 'Deactivate ticket upon receiving drink',
  ticketUnactive: 'Ticket unactive',
  ticketRefundedExplanation: 'This ticket has been refunded',
  ticketAlreadyUsedExplanation: 'This ticket has already been used',
  ticketUsedTimeAgoExplanation: 'Time passed since ticket used',
  yourOneTimeCode: 'Your one-time code',
  yourOneTimeCodeGreeting: 'Hello! This is your one-time code:',
  yourOneTimeCodeExplanation: 'Use this code to log into Norda TIckets',
  moreInfo: 'More info',
  birthdateConfig: 'Birthdate settings',
  noEventsInList: 'No events in your list',
  needYourBirthdateTitle: 'We need your birthdate!',
  save: 'Save',
  alreadyHaveSetBirthdateTitle: 'We already have your birthdate',
  ifIncorrectGetInContactWithHelp: 'If this is incorrect, get in contact with help@nordatickets.com to rectify it.',
  accessControlTickets: 'Access tickets',
  requiredToGetIntoTheEvent: 'Required to get into the event',
  walletTicketAccessExplanation: 'Use it to access the event',
  deactivateTicketOnAccessExplanation: 'Deactivate ticket when asked to access the event',
  download: 'Download',
  legalNotice: 'Legal notice',
  privacyPolicy: 'Privacy policy',
  cookiePolicy: 'Cookie policy',
  generalConditions: 'General conditions',
  enterYourAnswer: 'Enter your answer',
  addToCart: 'Add to cart',
  showFormSubmit: 'Information added',
  selectAnOption: 'Select an option',
  fullname: 'Name and surnames',
  registrations: 'Registrations',
  loginRequired: 'Requires login',
  logInToSeeTickets: 'Log in to see your tickets',
  logInToSeeReceipts: 'Log in to see your receipts',
  logInToProceedPurchase: 'Log in to continue with the purchase',
  tickets: 'Tickets',
  tastings: 'Tastings',
  drinks: 'Drinks',
  additionalInfo: 'Additional info',
  conditions: 'Conditions',
  additionalInfoAndConditions: 'Additional info and conditions'
};
